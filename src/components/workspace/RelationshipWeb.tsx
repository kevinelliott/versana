...
import { Users, Info, Settings, MessageSquare, ArrowLeft, Send } from 'lucide-react';
...
export default function RelationshipWeb() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const systemPrompt = selectedNodeId ? `Act as ${nodes.find(n => n.id === selectedNodeId)?.data.label}, a character in my story. Respond in character based on the author's worldbuilding. Keep your responses concise and engaging.` : '';
    ...
    useEffect(() => {
        if (activeWorkspace?.board_state?.relationshipWeb) {
            const rw = activeWorkspace.board_state.relationshipWeb;
                                            ...
                                            <div className={styles.chatMessages}>
                                                {messages.length === 0 && (
                                                    <p style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.85rem', marginTop: 'auto', marginBottom: 'auto' }}>
                                                        Start chatting with {selectedNode.data.label}...
                                                    </p>
                                                )}
                                                {messages.map((m: any) => (
